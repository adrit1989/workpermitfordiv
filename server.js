...
app.post('/api/save-permit', authenticateToken, upload.any(), async (req, res) => {
    try {
        ...
        if (req.body.Desc && req.body.Desc.length > 500) { return res.status(400).json({ error: "Description too long (max 500 chars)" }); }
    ...
    try {
        ...
        if (renewals.length === 1) {
            ...
            if (r1.status === 'pending_review' || r1.status === 'pending_approval') {
                if (action === 'reject') {
                    r1.status = 'rejected'; r1.rej_by = user; r1.rej_reason = "Rejected along with Main Permit";
                } 
